...
SELECT C.MEDIDOR_ID, C.DIA_NUMERO, FECHA, CANTIDAD_LECTURA, L.MAX_VOLUMEN, GET_MAX_VOLUMEN(C.MEDIDOR_ID, C.DIA_NUMERO) MAX_VOLUMEN_CALC FROM V_CALENDARIO_UF_MEDIDOR C LEFT JOIN V_LECUTURA_AGRUPACION_DIARIA L ON (C.DIA_NUMERO = L.DIA_NUMERO AND C.MEDIDOR_ID = L.MEDIDOR_ID);
